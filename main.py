...
def plot_radial(figure, figure_canvas_r, t_str, a, conc, D, n_point_r):
    ...
    axes.title.set_text(f'Concentration along radius at t = {str(t)} seconds')
    ...
    for time in t:
        ...
        y_r = radial_profile(time, a, D, conc, int(n_point_r))
        axes.plot(x_r, y_r)
...
simulation_plot_frame = ttk.Frame(notebook, width=root.winfo_width(), height=root.winfo_height())
