...
# calculate button
calc_c = tk.Button(calc_frame, text="Calculate", command=lambda: solve(r_c.get(), t_c.get(), a_c.get(), conc_c.get(), D_c.get(), result_label_c))
...
radial_plot_frame = ttk.Frame(notebook, width=1920, height=1080)
...
dif_label_r.grid(column=0, row=1, padx=pdx, pady=pdy)

D_r = tk.StringVar()
dif_entry_r = tk.Entry(radial_plot_frame, textvariable=D_r)
dif_entry_r.grid(column=1, row=1, sticky='EW', padx=pdx, pady=pdy)

# Radius entry

dif_label_r = tk.Label(radial_plot_frame, text="Radius")
dif_label_r.grid(column=2, row=1, padx=pdx, pady=pdy)

a_r = tk.StringVar()
radius_entry_r = tk.Entry(radial_plot_frame, textvariable=a_r)
radius_entry_r.grid(column=3, row=1, sticky='EW', padx=pdx, pady=pdy)

# initial concentration entry

init_conc_label_r = tk.Label(radial_plot_frame, text="Initial Concentration")
init_conc_label_r.grid(column=0, row=2, sticky='EW', padx=pdx, pady=pdy)

conc_r = tk.StringVar()
conc_entry_r = tk.Entry(radial_plot_frame, textvariable=conc_r)
conc_entry_r.grid(column=1, row=2, sticky='EW', pady=pdy, padx=pdx)

# Time entry

time_label_r = tk.Label(radial_plot_frame, text="Time")
time_label_r.grid(column=2, row=2)

t_r = tk.StringVar()
time_entry_r = tk.Entry(radial_plot_frame, textvariable=t_r)
time_entry_r.grid(column=3, row=2, sticky='EW', pady=pdy, padx=pdx)
...
calc_r.grid(column=0, columnspan=2, row=5)
